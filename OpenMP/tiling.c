...
// Uso: OMP_NUM_THREADS=N ./tiling <arquivo.png> <iteracoes> <sigma> <repeticoes>

int main(int argc, char* argv[]) {

    if (argc != 5) {
        fprintf(stderr, "Uso: OMP_NUM_THREADS=N %s <arquivo.png> <iteracoes> <sigma> <repeticoes>\n", argv[0]);
        fprintf(stderr, "Exemplo: OMP_NUM_THREADS=4 %s image_4096x4096.png 100 1.0 5\n", argv[0]);
        return 1;
    }
    ...
    for (int r = 0; r < repeticoes; r++) {
        ...
        for (int i = 0; i < iterations; i++) {
            gaussian_blur_tiling(src, dst, kernel, height, width);
    ...
    // ─── Salva resultado ───────────────────────────────────────────────────
    char output[64];
    snprintf(output, sizeof(output), "output_tiling_%dthreads.png", omp_get_max_threads());
    save_img(output, src, height, width);
