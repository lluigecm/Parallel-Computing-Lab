...
// Uso: OMP_NUM_THREADS=N ./openmp <arquivo.png> <iteracoes> <sigma> <repeticoes>
...
int main(int argc, char* argv[]) {
    ...
    for (int r = 0; r < repeticoes; r++) {
        ...
        for (int i = 0; i < iterations; i++) {
            gaussian_blur_openmp(src, dst, kernel, height, width);
    ...
    // ─── Salva resultado ───────────────────────────────────────────────────
    char output[64];
    snprintf(output, sizeof(output), "output_openmp_%dthreads.png", omp_get_max_threads());
